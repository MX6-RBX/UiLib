...
-- classes
local insertGui = game.CoreGui
local library = {} -- main
local page = {}
local section = {}
if game:GetService("RunService"):IsStudio() then
	insertGui = player.PlayerGui
end
do
			...
			utility:Create("ImageLabel", {
				...
				utility:Create("ImageLabel", {
					...
					utility:Create("ScrollingFrame", {
						...
						CanvasSize = UDim2.new(0, 0, 0, (40* 0)-5),
						ScrollBarThickness = 3,
						ScrollBarImageColor3 = themes.ScrollBarColor,
						ZIndex = 3,
						BorderSizePixel = 0
					}, {
	...
	function library:Notify(title, text, callback)
		...
		local padding = 10
		local textSize = game:GetService("TextService"):GetTextSize(text, 12, Enum.Font.Arimo, Vector2.new(math.huge, 16))
	...
	function section:addSlider(title, default, min, max, callback)
		...
		textbox.FocusLost:Connect(function()
			if not tonumber(textbox.Text) then
				local Input = tonumber(textbox.text) or value
				local Clamp = math.clamp(Input,min,max)
				value = self:updateSlider(slider, nil, Clamp, min, max)
				callback(value)
			end
		end)
		...
		textbox:GetPropertyChangedSignal("Text"):Connect(function()
			...
			elseif not allowed[text] then	
				local Input = tonumber(text) or value
				local Clamp = math.clamp(Input,min,max)
				value = self:updateSlider(slider, nil, Clamp, min, max)
				callback(value)
			end
	...
	function library:SelectPage(page, toggle)
		...
		if toggle then
			...
			button.Title.Font = Enum.Font.Arimo
	...
	function section:updateDropdown(dropdown, title, list, callback)
		...
		utility:Tween(dropdown, {Size = UDim2.new(1, 0, 0, (entries == 0 and 30) or math.clamp(entries, 0, 3) * 34 + 48)}, 0.3)
